	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 km C:/Users/Tim/Dropbox/neuron/optimisetraub/model/km.mod\n");
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   alpha = ( 0.02 / ( 1.0 + exp ( ( - ( _lv + vshift ) - 20.0 ) / 5.0 ) ) ) * taumod ;
   beta = ( 0.01 * exp ( ( - ( _lv + vshift ) - 43.0 ) / 18.0 ) ) * taumod ;
    return 0; }
