	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 kdr C:/Users/Tim/Dropbox/neuron/optimisetraub/model/kdr.mod\n");
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   minf = 1.0 / ( 1.0 + exp ( ( - ( _lv + vshift ) - 29.5 ) / 10.0 ) ) ;
   if ( _lv < - 10.0 ) {
     mtau = ( 0.25 + 4.35 * exp ( ( ( _lv + vshift ) + 10.0 ) / 10.0 ) ) * taumod ;
     }
   else {
     mtau = ( 0.25 + 4.35 * exp ( ( - ( _lv + vshift ) - 10.0 ) / 10.0 ) ) * taumod ;
     }
